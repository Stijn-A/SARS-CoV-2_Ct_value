...
histogram_agegroup <- ggplot(data = data_sgtf_long,
                             aes(x = Ct, color = age_group10, fill = age_group10)) +
  geom_area(aes(y = after_stat(count)), binwidth = 1, stat = "bin", alpha = 0.1, position = "identity") +
  #geom_freqpoly(binwidth = 1,  alpha=0.5, position = 'identity') +
  scale_color_brewer(palette = "Blues", direction = -1, guide = "none") +
...
boxplot_agegroup <- ggplot(data = data_sgtf_long,
  ...
  scale_fill_brewer(palette = "Blues", guide = guide_legend(nrow = 2), direction = -1) +
  scale_y_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
plot_agegroup <- cowplot::plot_grid(histogram_agegroup, boxplot_agegroup, align = "v", nrow = 2, rel_heights = c(0.4,0.6))
plot_agegroup
...
plot_sex <- cowplot::plot_grid(histogram_sex, boxplot_sex, align = "v", nrow = 2, rel_heights = c(0.4,0.6))
plot_sex
...
plot_lab <- cowplot::plot_grid(histogram_lab, boxplot_lab, align = "v", nrow = 2, rel_heights = c(0.4,0.6))
plot_lab
...
histogram_time <- ggplot(data = data_sgtf_long,
                             aes(x = Ct, color = time_of_testing, fill = time_of_testing)) +
  geom_area(aes(y = after_stat(count)), binwidth = 1, stat = "bin", alpha = 0.1, position = "identity") +
  scale_color_brewer(palette = "Purples", direction = -1, guide = "none") +
  scale_fill_brewer(palette = "Purples",  direction = -1, guide = "none") +
  scale_x_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
boxplot_time <- ggplot(data = data_sgtf_long,
                           aes(x = time_of_testing, y = Ct, fill = time_of_testing)) +
  geom_boxplot(alpha=0.9)  +
  scale_fill_brewer(palette = "Purples", guide = guide_legend(nrow = 1), direction = -1) +
  scale_y_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
plot_time <- cowplot::plot_grid(histogram_time, boxplot_time, align = "v", nrow = 2, rel_heights = c(0.4,0.6))
plot_time
...
histogram_variant <- ggplot(data = data_sgtf_long,
                             aes(x = Ct, color = variant_short, fill = variant_short)) +
  geom_area(aes(y = after_stat(count)), binwidth = 1, stat = "bin", alpha = 0.1, position = "identity") +
  scale_color_brewer(palette = "Set3", direction = -1, guide = "none") +
  scale_fill_brewer(palette = "Set3",  direction = -1, guide = "none") +
  scale_x_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
boxplot_variant <- ggplot(data = data_sgtf_long,
                           aes(x = variant_short, y = Ct, fill = variant_short)) +
  geom_boxplot(alpha=0.9)  +
  scale_fill_brewer(palette = "Set3", guide = guide_legend(nrow = 2), direction = -1) +
  scale_y_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
plot_variant <- cowplot::plot_grid(histogram_variant, boxplot_variant, align = "v", nrow = 2, rel_heights = c(0.4,0.6))
plot_variant
...
histogram_onset <- ggplot(data = data_sgtf_long,
                             aes(x = Ct, color = time_since_symp_onset_cat, fill = time_since_symp_onset_cat)) +
  geom_area(aes(y = after_stat(count)), binwidth = 1, stat = "bin", alpha = 0.1, position = "identity") +
  #geom_freqpoly(binwidth = 1,  alpha=0.5, position = 'identity') +
  scale_color_brewer(palette = "YlOrRd", guide = "none", direction = -1) +
  scale_fill_brewer(palette = "YlOrRd", guide = "none", direction = -1) +
  scale_x_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
boxplot_onset <- ggplot(data = data_sgtf_long,
                           aes(x = time_since_symp_onset_cat, y = Ct, fill = time_since_symp_onset_cat)) +
  #geom_violin() +
  geom_boxplot(alpha=0.9)  +
  scale_fill_brewer(palette = "YlOrRd", guide = guide_legend(nrow = 1), direction = -1) +
  scale_y_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
histogram_previous_infection <- ggplot(data = data_sgtf_long,
                          aes(x = Ct, color = fct_previous_infection, fill = fct_previous_infection)) +
  geom_area(aes(y = after_stat(count)), binwidth = 1, stat = "bin", alpha = 0.9, position = "identity") +
  #geom_freqpoly(binwidth = 1,  alpha=0.5, position = 'identity') +
  scale_color_brewer(palette = "Greens", guide = "none", direction = 1) +
...
boxplot_previous_infection <- ggplot(data = data_sgtf_long,
                        aes(x = fct_previous_infection, y = Ct, fill = fct_previous_infection)) +
  #geom_violin() +
  geom_boxplot(alpha=0.9)  +
  scale_fill_brewer(palette = "Greens", guide = guide_legend(nrow = 1), direction = 1) +
  scale_y_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
...
histogram_previous_infection_time <- ggplot(data = data_sgtf_long %>% filter(!is.na(fct_prev_inf_time)),
                                       aes(x = Ct, color = fct_prev_inf_time, fill = fct_prev_inf_time)) +
  geom_area(aes(y = after_stat(count)), binwidth = 1, stat = "bin", alpha = 0.1, position = "identity") +
  scale_color_brewer(palette = "Spectral", guide = "none", direction = -1) +
...
boxplot_previous_infection_time <- ggplot(data = data_sgtf_long %>% filter(!is.na(fct_prev_inf_time)),
                                     aes(x = fct_prev_inf_time, y = Ct, fill = fct_prev_inf_time)) +
  #geom_violin() +
  geom_boxplot(alpha=0.9)  +
  scale_fill_brewer(palette = "Spectral", guide = guide_legend(nrow = 2), direction = -1) +
  scale_y_continuous(
    breaks = seq(0,30,5),
    minor_breaks = NULL,
    # decrease gap between x-axis and plot area (but keep padding on top)
  ...
  facet_wrap(vars(Target), nrow = 1) +
  labs(x = "time since previous infection (days)") +
  coord_flip(ylim = c(1,30)) +
  theme_minimal() +
  theme(legend.position = "bottom",
        # remove facet labels
...
plot_previous_infection_time
rm(histogram_previous_infection_time, boxplot_previous_infection_time)
...
histogram_vaccination <- ggplot(data = data_sgtf_long,
                                aes(x = Ct, color = fct_vacc_status, fill = fct_vacc_status)) +
  geom_area(
    aes(y = after_stat(count)),
    binwidth = 1,
    stat = "bin",
...
boxplot_vaccination <- ggplot(data = data_sgtf_long,
                              aes(x = fct_vacc_status, y = Ct, fill = fct_vacc_status)) +
  #geom_violin() +
  geom_boxplot(alpha = 0.9)  +
  ...
  scale_x_discrete(# decrease gap between x-axis and plot area (but keep padding on top)
    expand = expansion(mult = c(0, .05))) +
  facet_wrap(vars(Target), nrow = 1) +
...
histogram_vaccination_time <- ggplot(data = data_sgtf_long %>% filter(!is.na(fct_vacc_time)),
...
boxplot_vaccination_time <- ggplot(data = data_sgtf_long %>% filter(!is.na(fct_vacc_time)),
  ...
  scale_x_discrete(# decrease gap between x-axis and plot area (but keep padding on top)
    expand = expansion(mult = c(0, .05))) +
  facet_wrap(vars(Target), nrow = 1) +
  labs(x = "time since vaccination (days)") +
  coord_flip(ylim = c(0, 30)) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
figure_covariates <- cowplot::plot_grid(plot_agegroup,
                   plot_sex,
                   plot_lab,
                   plot_onset,
                   plot_variant,
