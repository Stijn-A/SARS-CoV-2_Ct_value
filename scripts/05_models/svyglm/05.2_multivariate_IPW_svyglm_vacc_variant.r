...
fun_ips <- function(gam, data) {
  ...
  tab_ps <- as_tibble(predict(gam, type = "response")) %>%
    ...
    bind_cols(.,
              data %>% 
                select(fct_vacc_status)) %>%
    pivot_longer(cols = !fct_vacc_status) %>%
...
tab_vacc_variant %>% write_rds("cache/tab_vacc_variant.rds")

tab_vacc_variant <- tab_vacc_variant %>%
  mutate(
    ips = map2(
      .x = gam,
      .y = data,
...
for (i in c("Alpha", "Delta", "Omicron BA.1", "Omicron BA.2", "Omicron BA.4/5")) {
  ...
  assign(paste0("boxplot_vacc_",i), boxplot_fct_vacc_status)
}

boxplot_vacc_variant <- plot_grid(
  boxplot_vacc_Alpha,
  boxplot_vacc_Delta,
  `boxplot_vacc_Omicron BA.1`,
  `boxplot_vacc_Omicron BA.2`,
  `boxplot_vacc_Omicron BA.4/5`,
  ncol = 1
)

boxplot_vacc_variant %>%
  write_rds(file = "output/boxplot_vacc_variant.rds")
...
fun_plot_est <- function(svymod, variant) {
  plot_outcomes_fct_vacc_status <-  svymod %>%
    map(.f = \(x) {
      ...
      x %>% tidy(conf.int = T) %>%
        ...
        mutate(
          term = str_remove(term, "fct_vacc_status") %>%
            factor(levels = rev(data_ct$fct_vacc_status %>% levels)),
          term_c = fct_recode(
            term,
...
plot_vacc_variant <- tab_vacc_variant %>% 
  pull(plot) %>%
...
#   
#   
  #saveRDS("cache/mod_outcome_vacc_variant.rds")
...
plot_vacc_variant %>% 
  write_rds(file = "output/plot_vacc_variant.rds")
